...
export const TerminalGrid = memo(function TerminalGrid({
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      >
        {panes.map((pane, i) => {
          // Last row: if fewer items than cols, span remaining columns
          const isLastRow = i >= (rows - 1) * cols;
          const itemsInLastRow = count - (rows - 1) * cols;
          const isLastItem = i === count - 1;
          const colSpan =
            isLastRow && isLastItem ? cols - itemsInLastRow + 1 : 1;

          return (
            <div
              key={pane.id}
              className="group relative overflow-hidden bg-app"
            ...
            >
              {count > 1 && (
                <button
                  ...
                  style={{ backdropFilter: "blur(4px)", background: "var(--sidebar)" }}
